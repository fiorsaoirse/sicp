...
; Аппликативное вычисление - интерпретатор вычисляет все операнды перед тем, как "провалиться" на уровень ниже, p вычислить нельзя, т.к. получится бесконечная рекурсия, внутри p идет вычисление самого p
; Нормальное вычисление - значением будет 0, операнд p не вычислится никогда
